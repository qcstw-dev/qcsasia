 ...
 */
	...
	if($submited == '1')
	{
			...
			$sqlQuery = "UPDATE qcs_members SET password = '" . md5(md5($password) .  $resulta->date_creation) . "' WHERE email = '" . $email . "' AND resetkey = '" . $key . "'";
			$wpdb->query($sqlQuery);
	...
	<div id="primary" class="site-content">
		...
		?>
 		 ...
 		 <form method="post" action="">
                  ...
                  <p>
                  	<label for="submit">&nbsp;</label><input type="button" value="Reset" class="button" onClick = "var password = document.getElementById('password').value; var confirmPassword = document.getElementById('confirm-password').value; if(password == '' || password != confirmPassword) { window.alert('Your password cannot be empty and has to match with the confirmed password.'); return false;} else this.form.submit();"/>
                 </p>
