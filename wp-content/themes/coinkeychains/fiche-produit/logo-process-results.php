<?php 
					...
					if(!empty($tabIdLogoProcessWhichMatchString) || !empty($tabLogoString))
					{
							...
							if(!empty($tabIdLogoProcessWhichMatchString))
							{
								...
								// WE NEED TO USE DESC INSTEAD OF ASC, BUG ?
								$connected = new WP_Query(array('order' => 'DESC' , 'orderby' => 'menu_order' , 'connected_type' => 'logo_process_to_product', 'connected_items' => get_queried_object() , 'orderby' => 'PRODUCT_logo_process_sort') );
		                    		...
		                    		while ( $connected->have_posts() ) : $connected->the_post();
		                    			...
		                    			if(in_array(get_the_ID() , $tabIdLogoProcessWhichMatch))
		                    			{
			                    			...
			                    			$tabLogoProcess[$i]['title'] = get_the_title();
			                       			...
			                       			// echo "title = " . $tabLogoProcess[$i]['title'] ."<br/>";
			                       			
			                       			
			                       			$tabLogoProcess[$i]['tabImage'] = rwmb_meta( "PRODUCT_tab_image_process_" . $j , 'type=image&size=large' , $idProduct);
							...
							//	echo "AAAAAAAAAAA";
			                    		...
			                    		$logoProcessName = rwmb_meta( 'PRODUCT__logo_process_logo_process' , $idProduct);
			               
			                    		
			                    		if(in_array($logoProcessName , $tabLogo))
			                    		{
			                    			
			                    			// echo "i = " .$i;
			                    			// echo "title = " . get_the_title();
			
			                       			$tabLogoProcess[$j]['title'] = get_the_title();
			                       			$tabLogoProcess[$j]['content'] = get_the_content();
			                       			$tabLogoProcess[$j]['link'] = get_permalink($post->ID);
			                       			
			                       			$tabLogoProcess[$j]['tabImage'] = rwmb_meta( "PRODUCT_tab_image_process_" . ($i + 1), 'type=image&size=large' , $idProduct);
			                       			$j++;
			                    		}
